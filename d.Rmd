...
Price of diamonds in this data set is capped at around \$18,800. Here are the top 2 most valuable for each quality of cut. The numbers are very similar.
...
This ceiling can hide the real connection between the variables, so to lessen that effect I'm going to filter out those diamonds that are larger or equal to 1.75 Ct.
...
The numbers are still pretty similar with higher quality diamonds, but we don't want to filter out too much of the data.
...
Let's explore the relationship between the carat and the price more.
...
Residuals here tell the difference between what the linear regression model predicted and its actual value.
...
paste0("The formula given by linear model is ",
       ...
       " % of the variation of a diamond's price.")
```

Formula works well when residuals are close to zero. But especially for those with either very bad or very good quality of cut and color the predictions are much worse.
...
Residuals represents the price where the effect of diamond's size has been removed. That's why we can now see better the actual impact that quality of cut has to the price. Same is true for the color of the diamond, but to keep things simple, I won't show those correlations.

This table shows the impact of some residuals and sizes to the price.
...
Diamonds is a classic of data analysis for a reason. It shows how superficial analysis can lead to a completely wrong conclusion.

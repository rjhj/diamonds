...
Price of diamonds in this data set is capped at around $18,000. Here are
the top 3 most valuable for each quality of cut. The numbers are very similar.
...
This ceiling can hide the real connection between the variables, so in analysis
I'm going filter out some of the biggest diamonds (>= 1.75). Ceiling will still
affect the results, but at least a little bit less.
...
ggplot(d, aes(price, after_stat(density), colour = cut)) +
  ...
  labs(title = "Surpirising result: fair diamonds are more valuable than ideal",
       caption = "Density plot: the area under each curve is one.")
```


```{r}
ggplot(d, mapping = aes(x = price))+
  geom_boxplot(aes(y = cut)) +
